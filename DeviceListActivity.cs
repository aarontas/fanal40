...
namespace com.xamarin.samples.bluetooth.bluetoothchat
{
    ...
    [Activity(Label = "@string/select_device",
                ConfigurationChanges = Android.Content.PM.ConfigChanges.KeyboardHidden | Android.Content.PM.ConfigChanges.Orientation)] //Se vuelve loco y rompe con los permisos si quito esto
    public class DeviceListActivity : Activity
    {
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            // Initialize the button to perform device discovery		//prueba ast el boton del menu emergente para seguir buscando dispositivos	
            /*var scanButton = FindViewById<Button>(Resource.Id.button_scan);
            scanButton.Click += (sender, e) =>
            {
                DoDiscovery();
                (sender as View).Visibility = ViewStates.Gone;
            };*/ //prueba ast me cargue el boton

            // Initialize array adapters. One for already paired devices and
            // one for newly discovered devices
            var pairedDevicesArrayAdapter = new ArrayAdapter<string>(this, Resource.Layout.device_name);
            newDevicesArrayAdapter = new ArrayAdapter<string>(this, Resource.Layout.device_name); //prueba ast limpia codigo y mira a ver si tu lista no es redundante

            //Initialize array adapter. This will use in a ListView
            var pairedDevicesArrayListView = new ArrayAdapter<string>(this, Resource.Layout.device_name_list);
           ...
           /* var newDevicesListView = FindViewById<ListView>(Resource.Id.new_devices);
            ...
            newDevicesListView.ItemClick += DeviceListView_ItemClick;*/ //Prueba ast me cargue lo del nuesvos dispositivos
            ...
            {
                //FindViewById(Resource.Id.title_paired_devices).Visibility = ViewStates.Visible; //prueba ast no se que hace esto
                foreach (var device in pairedDevices)
                {
                    pairedDevicesArrayAdapter.Add(device.Name + "\n" + device.Address);
                    pairedDevicesArrayListView.Add(device.Name + "\n" + device.Address);
                }
            }
            else
            {
                ...
                pairedDevicesArrayListView.Add(noDevices);
        ...
        void DoDiscovery()
        {
            ...
            //FindViewById<View>(Resource.Id.title_new_devices).Visibility = ViewStates.Visible; //prueba ast no se que hace esto
        ...
        void DeviceListView_ItemClick(object sender, AdapterView.ItemClickEventArgs e)
        {
            ...
            btAdapter.CancelDiscovery();
            //Prueba ast. Aqui le preguntamos si quiere conectarse
            // Get the device MAC address, which is the last 17 chars in the View
            var info = ((TextView)e.View).Text;
            var nameDevice = info.Substring(0, (info.Length - 19));
            var address = info.Substring(info.Length - 17);

            var alert = new AlertDialog.Builder(this);
            alert.SetTitle("Connect Device");
            alert.SetMessage("Do you want connect to: " + nameDevice);
            alert.SetPositiveButton("Yes", (senderAlert, arg) => { IntentConnect(address); });
            alert.SetNegativeButton("No", (senderAlert, arg) => { });
            alert.Show();

        }

        private void IntentConnect(string address)
        {
